...
```bash
curl -X GET 'https://tkrjfifrwttikwsghuju.supabase.co/rest/v1/messages?session_id=eq.123e4567-e89b-12d3-a456-426614174000&order=created_at.asc&limit=50' \
...
```bash
curl -X POST 'https://tkrjfifrwttikwsghuju.supabase.co/rest/v1/messages' \
  -H 'apikey: SUA_ANON_KEY' \
  -H 'Authorization: Bearer SUA_SERVICE_KEY' \
